...
jobs.monitoring = (cb) => {
    Disk.Monitoring.forEach((disk) => {
        ...
        if (disk.use > config.AlertStorage) { // add  && disk.use !== 100 in if () to filter loop disk
            slack.send(`storage ${disk.fs} use more than ${config.AlertStorage}%`, function (err, res, body) {
                if (err) throw err;
